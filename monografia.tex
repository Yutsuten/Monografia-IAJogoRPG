...
\newpage % Coloca o conteúdo numa nova página
\chapter{INTRODUÇÃO}
	O crescimento dos jogos eletrônicos nos últimos anos permitiu aumentar o alcance do público alvo deste mercado.
	Apesar disto, ainda é possível notar que muitas pessoas acreditam que jogos eletrônicos são para crianças ou pessoas "desocupadas"{}.
	...
	existem áreas dentro de IA em jogos que ainda estão inexploradas \cite{panoramaAI}.
...
\chapter{O JOGO}
	...
	Os passos do AG aqui descritos irão detalhar o que foi introduzido na seção \textit{\ref{ssec:processo}} Processo (do AG).
	...
	A saída do primeiro neurônio é a que calcula a chance daquele personagem se tornar alvo.
	Quanto mais próximo de 1, maior a prioridade,
	e quanto mais próximo de 0, menor.
	Os outros 11 neurônios representam, cada um, a prioridade de utilizar um determinado comando naquele personagem, caso ele vire alvo.
	...
	foi criada uma estrutura que une "alvo"{} a "comando"{} em uma única variável de prioridade,
	se segue a fórmula \eqref{eq:targetCommand} a seguir:
	
	\begin{equation}
		\textrm{Prioridade} = 1,5 \times \textrm{Chance de virar alvo} + \textrm{Prioridade comando}
...
\chapter{CONCLUSÃO}
	...
	E também é possível criar uma variação deste trabalho que use duas instâncias do AG,
	um para cada equipe.
	Desta forma, as duas equipes estariam competindo para melhorar seu desempenho.

	Como já falado no início deste trabalho, jogos é uma área que está em constante crescimento no mercado.
	O desenvolvimento dela com áreas como IA se motra ter um grande potencial, e com um horizonte que, mesmo hoje, ainda não se sabe onde termina.
