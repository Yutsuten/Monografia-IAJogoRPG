...
\chapter{CONCEITOS}
	...
	Uma breve descrição e alguns exemplos serão feitos sobre cada um deles.
		...
		por conter histórias interessantes e muitas vezes engraçadas\footnote{Exemplo de áudio de um jogo RPG de mesa: \url{https://www.youtube.com/watch?v=x-toFdM6Cfc}}.
		
		% Introdução ao exemplo prático de Dungeons & Dragons (motivo: relacionado com o trabalho)
		A fim de se ter um exemplo prático deste tipo de jogo,
		na próxima seção explicará detalhes de um RPG de mesa clássico,
		chamado \textit{Dungeons \& Dragons}.
		
			\FloatBarrier
			\subsubsection{Dungeons \& Dragons}
			<Texto sobre Dungeons \& Dragons.>
			% Descrição do jogo (mais conhecido)
			% Regras
		...
		% Dizer que normalmente os jogos normalmente não possuem apenas um estilo, mas sim híbridos
		% Informar que o estilo escolhido para este trabalho foi o RPG
			...
			% Imagem de RPG de Ação (Final Fantasy)
